/* RSVP Reader: The Alignment-Industrial Complex */
...
(function () {
  ...
  function showWord() {
    ...
    if (!w) {
      ...
      // Chapter loaded but word missing; position is stale, so reset to chapter start
        pos = chapterOffsets[ch];
        w = getWord(pos);
        if (!w && pos > 0) { pos = 0; w = getWord(pos); }
        if (!w) return; // give up
      } else {
      // Chapter not loaded yet; show the loading state
        wordPre.textContent = "";
        wordOrp.textContent = "";
